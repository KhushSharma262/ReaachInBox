...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      // ── Step 2: Fetch created emails and enqueue BullMQ jobs ──────────────
      ...
      // Enqueue all jobs — errors here are caught and logged but don't fail
      ...
      for (const email of scheduledEmails) {
        try {
          ...
          logger.error(
            ...
            'Failed to enqueue email job — will be recovered on worker startup',
          );
        }
      }
